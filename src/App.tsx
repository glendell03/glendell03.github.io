...
import { motion, useScroll, useTransform, useSpring, useVelocity, useAnimationFrame, useMotionValue, AnimatePresence, useMotionValueEvent } from 'motion/react';
import { Github, Linkedin, Mail, Globe, Phone, ExternalLink, Hammer, Users, Zap, MapPin, Briefcase, Tag, ArrowRight } from 'lucide-react';
...
const projects = [
  ...
  { name: "IPFSR Website", role: "Frontend Developer", date: "2024", link: "https://ipfsr-website.vercel.app/" },
  { name: "Delta Storage", role: "Full Stack Developer", date: "2023 - 2024", link: "https://www.delta.storage/" },
  { name: "Efti", role: "Full Stack Developer", date: "2023", link: "https://efti.vercel.app/" },
  { name: "DeezJobs", role: "Full Stack Developer", date: "2022", link: "https://www.deezjobs.xyz/" },
  { name: "Ain't Board", role: "Full Stack Developer", date: "2021", link: "https://aintboard.com/" },
];

const skills = {
  languages: ["Typescript/Javascript", "Python", "Go", "Rust", "Move"],
  technologies: ["React/Next.js", "React Native", "Vue", "Angular", "Redux", "Zustand", "Tanstack Query", "Tailwind", "Node.js", "Express", "NestJS", "FastAPI", "MySQL", "Postgres", "MongoDB", "Prisma", "Stripe", "Solana", "Web3.js"]
};
...
const VelocityMarquee = ({ children, baseVelocity = 2 }: { children: React.ReactNode, baseVelocity?: number }) => {
  ...
  return (
    <div className="relative h-screen w-full bg-zinc-950 text-zinc-50 flex flex-col justify-between p-8 md:p-16 z-10 overflow-hidden">
      ...
      <div className="flex justify-between items-start relative z-10">
        <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="text-sm font-mono tracking-widest uppercase text-zinc-400">PORTFOLIO '26</motion.span>
        <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="text-sm font-mono tracking-widest uppercase text-zinc-400">METRO MANILA, PH</motion.span>
      </div>
      
      <div className="flex flex-col relative z-10">
        <TextReveal text="GLENDELL" className="text-[15vw] font-display font-bold leading-[0.85] tracking-tighter" />
        <TextReveal text="BRINGINO" className="text-[15vw] font-display font-bold leading-[0.85] tracking-tighter text-zinc-500" />
        <motion.div 
        ...
        >
          ...
          <span className="text-xl md:text-3xl font-light tracking-widest uppercase">SOFTWARE ENGINEER</span>
        </motion.div>
      </div>
    </div>
  )
  ...
  return (
    <div className="relative min-h-screen w-full bg-zinc-50 text-zinc-950 flex flex-col justify-center p-8 md:p-16 z-20 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
      <div className="max-w-6xl mx-auto w-full">
        <h2 className="text-sm font-mono tracking-widest uppercase text-zinc-500 mb-12">01 — Profile</h2>
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div>
            <TextReveal text="I build websites, apps, and little tools that actually make people's lives easier." className="text-3xl md:text-5xl font-medium leading-tight tracking-tight mb-8" />
            <motion.p 
              initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }}
              className="text-xl md:text-2xl text-zinc-500 font-light"
            >
              Fast? <span className="text-orange-600 font-medium">Yep.</span> Clean? <span className="text-orange-600 font-medium">Always.</span> Boring? <span className="text-orange-600 font-medium">Never.</span>
            </motion.p>
          </div>
          ...
          <div className="flex flex-col gap-12">
            <div className="grid grid-cols-2 gap-8">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <h4 className="text-5xl font-display font-bold text-zinc-900">4+</h4>
                <p className="text-sm font-mono text-zinc-500 uppercase mt-2">Years Experience</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}>
                <h4 className="text-5xl font-display font-bold text-zinc-900">100%</h4>
                <p className="text-sm font-mono text-zinc-500 uppercase mt-2">Commitment to Quality</p>
              </motion.div>
            </div>
            
            <div className="space-y-6">
              {[
                { icon: Hammer, title: "Build", desc: "I bring ideas to life by coding the backend logic and connecting it to the frontend — making apps that actually work and feel alive." },
                { icon: Users, title: "Collaborate", desc: "I'm a team player who loves sprints, daily check-ins, and pairing up to solve problems. Flexible but focused. No micromanaging, just trust." },
                { icon: Zap, title: "Optimize", desc: "I hunt down bugs and fix slow parts, delivering clean, fast, and high-quality code that runs smoothly." }
              ].map((item, i) => (
                <motion.div 
                  key={i}
                ...
                >
                  <h4 className="text-lg font-bold uppercase tracking-wider mb-2 flex items-center gap-2">
                    <item.icon className="w-5 h-5 text-orange-600 transform group-hover:scale-110 transition-transform" /> {item.title}
                  </h4>
...
const Projects = () => {
  return (
    <div className="relative min-h-screen w-full bg-zinc-100 text-zinc-950 flex flex-col justify-center p-8 md:p-16 z-[25] shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
      <div className="max-w-6xl mx-auto w-full">
        <h2 className="text-sm font-mono tracking-widest uppercase text-zinc-500 mb-12">02 — Projects</h2>
        
        <div className="flex flex-col border-t border-zinc-300">
          {projects.map((project, i) => (
            <Magnetic key={i} strength={0.1}>
              <a href={project.link} target="_blank" rel="noreferrer" className="group flex flex-col md:flex-row justify-between items-start md:items-center py-8 border-b border-zinc-300 hover:px-8 transition-all duration-500 relative overflow-hidden">
                ...
                <div className="flex items-center gap-8 z-10 group-hover:text-white transition-colors duration-300">
                  <span className="text-sm font-mono text-zinc-400 group-hover:text-orange-200">0{i + 1}</span>
                  <h3 className="text-3xl md:text-5xl font-display font-bold">{project.name}</h3>
                </div>
                
                <div className="flex items-center gap-8 mt-4 md:mt-0 z-10 group-hover:text-white transition-colors duration-300">
                  <p className="text-zinc-500 group-hover:text-orange-100">{project.role}</p>
                  <span className="text-sm font-mono bg-zinc-200 group-hover:bg-orange-500 px-3 py-1 rounded-full">{project.date}</span>
...
const KineticExperience = () => {
  ...
  const { imageUrl, loading } = useGeneratedImage(experiencePrompts[activeIndex] || experiencePrompts[0]);

  return (
    <div ref={containerRef} className="relative bg-zinc-950 text-zinc-50 z-30 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]" style={{ height: `${experiences.length * 100}vh` }}>
      <div className="sticky top-0 h-screen w-full overflow-hidden flex">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            exit={{ clipPath: "polygon(0 0, 100% 0, 100% 0, 0 0)", scale: 0.95 }}
            transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
            className="absolute inset-0 w-full h-full"
          >
            {imageUrl ? (
              <img src={imageUrl} alt="Experience Background" className="w-full h-full object-cover opacity-20 mix-blend-screen" />
            ) : (
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col h-full p-8 md:p-16">
          <h2 className="text-sm font-mono tracking-widest uppercase text-zinc-500 shrink-0 mb-8 md:mb-12">03 — Experience</h2>
          ...
          <div className="flex flex-col md:flex-row flex-1 min-h-0 gap-8 md:gap-16">
            ...
            <div className="w-full md:w-1/3 flex flex-col justify-center gap-8 border-l border-zinc-800 pl-8 relative">
              ...
              {experiences.map((exp, i) => (
                <div key={i} className={`transition-all duration-500 ${i === activeIndex ? 'opacity-100 translate-x-4' : 'opacity-40 hover:opacity-70'}`}>
                  <h3 className={`text-lg md:text-2xl font-display font-bold ${i === activeIndex ? 'text-orange-500' : 'text-zinc-50'}`}>{exp.company}</h3>
                  <p className="text-xs md:text-sm font-mono mt-1">{exp.date}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full md:w-2/3 flex flex-col justify-center relative">
            <AnimatePresence mode="wait">
              ...
              >
                <div>
                  {activeExp.link ? (
                    <a href={activeExp.link} target="_blank" rel="noreferrer" className="text-3xl md:text-6xl font-display font-bold text-zinc-50 flex items-center gap-4 hover:text-orange-500 transition-colors w-fit group">
                      {activeExp.company} <ExternalLink className="w-5 h-5 md:w-8 md:h-8 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                    </a>
                  ) : (
                    <h3 className="text-3xl md:text-6xl font-display font-bold text-zinc-50">{activeExp.company}</h3>
                  )}
                  <p className="text-xl md:text-3xl text-orange-500 mt-2 md:mt-4 font-display font-light">{activeExp.role}</p>
                </div>

                <div className="flex flex-wrap gap-3 md:gap-6 mt-2">
                  <span className="flex items-center gap-2 text-xs md:text-sm font-mono text-zinc-400 bg-zinc-900/50 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-zinc-800 backdrop-blur-sm"><MapPin className="w-3 h-3 md:w-4 md:h-4 text-orange-500"/> {activeExp.location}</span>
                  <span className="flex items-center gap-2 text-xs md:text-sm font-mono text-zinc-400 bg-zinc-900/50 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-zinc-800 backdrop-blur-sm"><Briefcase className="w-3 h-3 md:w-4 md:h-4 text-orange-500"/> {activeExp.type}</span>
                  <span className="flex items-center gap-2 text-xs md:text-sm font-mono text-zinc-400 bg-zinc-900/50 px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-zinc-800 backdrop-blur-sm"><Tag className="w-3 h-3 md:w-4 md:h-4 text-orange-500"/> {activeExp.industry}</span>
                </div>
                ...
                <ul className="flex flex-col gap-3 md:gap-4 mt-2 md:mt-4">
                  {activeExp.description.map((desc: string, j: number) => (
                    ...
                    >
                      <span className="text-orange-500 mt-1 md:mt-1.5 text-xs md:text-sm">▹</span>
                      {desc}
                    </motion.li>
                  ))}
                </ul>
  ...
  return (
    <div className="relative h-screen w-full bg-orange-600 text-zinc-950 flex flex-col justify-center overflow-hidden z-40 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
      <div className="absolute top-8 md:top-16 left-8 md:left-16 z-10">
        <h2 className="text-sm font-mono tracking-widest uppercase text-zinc-900 font-bold">04 — Technologies</h2>
      </div>
      
      <div className="flex flex-col gap-4 md:gap-8 transform -rotate-3 scale-110">
        <VelocityMarquee baseVelocity={-2}>
          {skills.technologies.slice(0, 10).map((tech, i) => <span key={i} className="text-6xl md:text-8xl font-display font-black uppercase mx-4 md:mx-8">{tech}</span>)}
        </VelocityMarquee>
        <VelocityMarquee baseVelocity={2}>
          {skills.technologies.slice(10).map((tech, i) => <span key={i} className="text-6xl md:text-8xl font-display font-black uppercase mx-4 md:mx-8 text-zinc-900/40">{tech}</span>)}
        </VelocityMarquee>
        <VelocityMarquee baseVelocity={-3}>
          {skills.languages.map((lang, i) => <span key={i} className="text-6xl md:text-8xl font-display font-black uppercase mx-4 md:mx-8">{lang}</span>)}
        </VelocityMarquee>
      </div>
    </div>
  )
  ...
  return (
    <div className="relative min-h-screen w-full bg-zinc-950 text-zinc-50 flex flex-col justify-between p-8 md:p-16 z-50 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
      <div className="max-w-6xl mx-auto w-full flex-1 flex flex-col justify-center">
        <TextReveal text="LET'S TALK." className="text-[10vw] font-display font-bold leading-none tracking-tighter mb-8 text-orange-500" />
        
        <div className="flex flex-col md:flex-row gap-8 md:gap-16 text-xl md:text-2xl font-light">
          <Magnetic strength={0.2}>
            <a href="mailto:glendell.bringino.work@gmail.com" className="flex items-center gap-4 hover:text-orange-500 transition-colors w-fit">
